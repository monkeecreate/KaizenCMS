...
class Form_twitter extends Form_Field
{
	...
	public function html() {
		global $objDB, $aConfig;

		$sError = false;
		$aValue = $this->value();

		$sConsumerKey = $objDB->getOne("SELECT `value` FROM `".$aConfig["database"]["prefix"]."settings`"
			." WHERE `tag` = ".$objDB->escape("twitter_consumer_key")
		);
		$sConsumerSecret = $objDB->getOne("SELECT `value` FROM `".$aConfig["database"]["prefix"]."settings`"
			." WHERE `tag` = ".$objDB->escape("twitter_consumer_secret")
		);

		if(!empty($aValue) && !empty($aValue["screen_name"])) {
			require_once(APP."helpers/twitteroauth.php");
