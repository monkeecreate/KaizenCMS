...
if(!$oDatabase->isConnected()) {
	$sFail = "Failed to connect to database.";
}

if($_POST["setup"] == 1) {
	$oDatabase->query("UPDATE `".$aConfig["database"]["prefix"]."settings` SET `value` = ".$_POST["title"]." WHERE `tag` = 'title'");
	$oDatabase->query("UPDATE `".$aConfig["database"]["prefix"]."settings` SET `value` = ".$_POST["contact"]." WHERE `tag` = 'email'");

	$sConfig = file_get_contents(APP."inc_config.php");
	...
	file_put_contents(APP."inc_config.php", $sConfig);

	if(!empty($_POST["admin_password"])) {
		$oDatabase->query("TRUNCATE TABLE `".$aConfig["database"]["prefix"]."users`");
		$sResults = $oDatabase->query("INSERT INTO `".$aConfig["database"]["prefix"]."users`"
			." SET `username` = ".$_POST["admin_username"]
			.", `password` = ".sha1($_POST["admin_password"])
			.", `fname` = ".$_POST["admin_fname"]
			.", `lname` = ".$_POST["admin_lname"]
			.", `email_address` = ".$_POST["admin_email"]
			.", `super` = 1"
	...
	$_POST["title"] = $oDatabase->getOne("SELECT `value` FROM `".$aConfig["database"]["prefix"]."settings` WHERE `tag` = 'site-title'");
	$_POST["contact"] = $oDatabase->getOne("SELECT `value` FROM `".$aConfig["database"]["prefix"]."settings` WHERE `tag` = 'contact-email'");

	$sAdmin = $oDatabase->getRow("SELECT * FROM `".$aConfig["database"]["prefix"]."users` WHERE `id` = 1");
