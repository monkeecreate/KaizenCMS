...
<form method="post" action="/admin/galleries/{$aGallery.id}/photos/manage/s/" enctype="multipart/form-data">
	{foreach from=$aPhotos item=aPhoto}
		<div style="float:left;width:300px;margin: 0 40px 15px 0;padding: 0 0 15px;border-bottom: 1px solid #aaa;">
			<img width="300px" src="/image/resize/?file=/uploads/galleries/{$aGallery.id}/{$aPhoto.photo}&width=300&height=300" class="image">
			<label>Title:</label>
			<input type="text" name="photo[{$aPhoto.id}][title]" maxlength="100" value="{$aPhoto.title|clean_html}" style="width:300px;">
